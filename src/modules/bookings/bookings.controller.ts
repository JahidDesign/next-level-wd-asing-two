...
export const updateBooking = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await BookingService.updateBookingStatus(req.user!, bookingId, status);

  if (status === 'cancelled') {
    return sendSuccess(res, 200, 'Booking cancelled successfully', result.booking);
  } else {
    return sendSuccess(res, 200, 'Booking marked as returned. Vehicle is now available', {
      ...result.booking,
      vehicle: {
